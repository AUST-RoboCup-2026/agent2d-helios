...
ShootGenerator::generate( const WorldModel & wm )
{
    ...
    // 根据距离动态调整采样密度：越近采样越多，避开守门员站立区域
const double dist_to_goal = M_first_ball_pos.dist( goal_l ); // 到左门柱距离（约等于到球门中心）
int DIST_DIVS;
if ( dist_to_goal < 15.0 ) {
    DIST_DIVS = 40;  // 近距离：精细采样
} else if ( dist_to_goal < 25.0 ) {
    DIST_DIVS = 30;  // 中距离
} else {
    DIST_DIVS = 20;  // 远距离：减少计算量
}

// 守门员感知采样：如果守门员站位明显偏向一侧，在另一侧（空档）加密采样
const AbstractPlayerObject * goalie = wm.getTheirGoalie();
if ( goalie && dist_to_goal < 20.0 ) {
    const double goalie_y = goalie->pos().y;
    const double goal_half_w = SP.goalHalfWidth();
    
    // 如果守门员在左侧（y < -1），则扩大右侧采样范围
    if ( goalie_y < -1.0 ) {
        goal_l.y = -goal_half_w; // 保持左侧
        goal_r.y = goal_half_w + 1.0; // 右侧向外扩展
    } else if ( goalie_y > 1.0 ) {
        // 守门员在右侧，扩展左侧
        goal_l.y = -(goal_half_w + 1.0);
        goal_r.y = goal_half_w;
    }
}
                             ...
                             const double & ball_move_dist )
{
          ...
          ++o )
    {
        ...
        if ( (*o)->goalie() )
        {
// 使用 ServerParam 动态计算小禁区风险线（禁区线前 4 米）
const double RISK_LINE_X = SP.theirPenaltyAreaLineX() - 4.0;

if ( maybeGoalieCatch( *o, course ) )
{
    // 远距离（禁区外）：放弃射门
    if ( wm.ball().pos().x < RISK_LINE_X ) 
    {
        return false; 
    }
    else 
    {
        // 小禁区附近：保留路线但标记风险，并在评分时惩罚
        course.goalie_never_reach_ = false; 
        // 可选：立即降低该路线的基础分，让系统优先选择守门员够不着的路线
        // course.score_base_ *= 0.7; // 如果添加了这个成员变量
    }
}
        ...
        // ==== 新增：踢球噪声鲁棒性检查 ====
    // 模拟踢球角度和速度的随机误差，检查球是否仍有可能进门
    const double angle_noise_deg = 3.0; // 3度角度误差
    const double speed_noise_ratio = 0.08; // 8%速度误差

    // 计算考虑噪声后的轨迹终点范围
    const AngleDeg angle_left = course.ball_move_angle_ - AngleDeg(angle_noise_deg);
    const AngleDeg angle_right = course.ball_move_angle_ + AngleDeg(angle_noise_deg);
    const double speed_min = course.first_ball_speed_ * (1.0 - speed_noise_ratio);
    const double speed_max = course.first_ball_speed_ * (1.0 + speed_noise_ratio);

    // 计算噪声影响下的落点范围（使用几何级数求和公式：距离 = 速度 / (1 - 衰减率)）
    const double ball_decay = ServerParam::i().ballDecay();
    const double reach_dist_min = speed_min / (1.0 - ball_decay);
    const double reach_dist_max = speed_max / (1.0 - ball_decay);

    Vector2D end_point_min = M_first_ball_pos + Vector2D::polar2vector(reach_dist_min, angle_left);
    Vector2D end_point_max = M_first_ball_pos + Vector2D::polar2vector(reach_dist_max, angle_right);

    // 检查噪声后的轨迹是否完全偏离球门（Y坐标超出球门半宽）
    const double goal_half_w = ServerParam::i().goalHalfWidth();
    if ( std::min(end_point_min.absY(), end_point_max.absY()) > goal_half_w + 0.5 &&
         std::max(end_point_min.absY(), end_point_max.absY()) > goal_half_w + 1.0 ) 
    {
        // 噪声导致球完全偏出球门
#ifdef DEBUG_PRINT
        dlog.addText( Logger::SHOOT,
                      "%d: xxx too sensitive to noise. target=(%.2f %.2f) y_range=[%.2f, %.2f]",
                      M_total_count,
                      target_point.x, target_point.y,
                      end_point_min.y, end_point_max.y );
#endif
        return false;
    }
    // ==== 噪声检查结束 ====
                                  ...
                                  Course & course )
{
    ...
    static const double CONTROL_AREA_BUF = 0.0;  // buffer for kick table
...
int bonus_step = ( in_penalty_area ? 1 : 0 ); //不给守门员太多预判帧
        if ( ! in_penalty_area )
        {
            bonus_step -= 1;
        }
...
ShootGenerator::evaluateCourses( const WorldModel & wm )
{
          ...
          ++it )
    {
        ...
        {
            double y_dist = std::max( 0.0, it->target_point_.absY() - 5.0 );
        ...
        score *= goalie_rate;
        score *= y_rate;
        // 1. 距离惩罚：距离球门越远，得分越低（20米外开始指数衰减）
        const double dist_to_goal = M_first_ball_pos.dist( SP.theirTeamGoalPos() );
        if ( dist_to_goal > 20.0 ) {
            double dist_penalty = std::exp( -std::pow(dist_to_goal - 20.0, 2.0) / 200.0 );
            score *= dist_penalty;
        }

        // 2. 射门步数惩罚：多步射门（需要调整站位）更容易被拦截
        if ( it->kick_step_ > 1 ) {
            score *= 0.6; // 多步射门打 6 折
        }

        // 3. 正对守门员惩罚：如果射门线路直接对准守门员，大幅降低得分
        if ( goalie && !it->goalie_never_reach_ ) {
            double angle_to_goalie = (goalie->pos() - M_first_ball_pos).th().abs();
            double angle_diff = ( it->ball_move_angle_.abs() - angle_to_goalie ).abs();
            if ( angle_diff < 20.0 ) { // 20度内视为"正对"
                score *= 0.3; // 打 3 折，大幅降低优先级
            }
        }
