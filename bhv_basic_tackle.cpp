...
Bhv_BasicTackle::execute( PlayerAgent * agent )
{
    ...
    // ==== 智能拦截决策 ====
    double success_rate = calculateTackleSuccessRate( agent );
    if ( success_rate < 0.5 )
    {
        executeBlock( agent );
        return true;
...
/*-------------------------------------------------------------------*/
double
Bhv_BasicTackle::calculateTackleSuccessRate( PlayerAgent * agent )
{
    const WorldModel & wm = agent->world();
    double rate = 0.85;
    if ( wm.ball().vel().r() > 2.5 ) rate -= 0.20;
    AngleDeg ball_dir = ( wm.ball().pos() - wm.self().pos() ).dir();
    double angle_diff = ( wm.self().body() - ball_dir ).abs();
    if ( angle_diff > 60.0 ) rate -= 0.25;
    return rate > 0.0 ? rate : 0.0;
}

/*-------------------------------------------------------------------*/
bool
Bhv_BasicTackle::executeBlock( PlayerAgent * agent )
{
    const WorldModel & wm = agent->world();
    Vector2D goal_pos( -52.5, 0.0 );
    if ( wm.self().side() == RIGHT ) goal_pos.x = 52.5;
    Vector2D block_pos = wm.ball().pos() + ( goal_pos - wm.ball().pos() ).setLengthVector( 2.5 );
    agent->doMove( block_pos.x, block_pos.y );
    return true;
}
